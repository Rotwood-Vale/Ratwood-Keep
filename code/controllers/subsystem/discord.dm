...
/datum/controller/subsystem/discord/Initialize(start_timeofday)
	...
	if(notifymsg)
		send2chat(new /datum/tgs_message_content("[notifymsg]"), CONFIG_GET(string/channel_announce_new_game)) // Sends the message to the discord, using same config option as the roundstart notification
