...
/mob/living/carbon/human/update_tod(todd)
	...
	if(todd == "day")
		if(HAS_TRAIT(src, TRAIT_DARKLING) && !HAS_TRAIT(src, TRAIT_NOSTAMINA) && !HAS_TRAIT(src, TRAIT_NOSLEEP) && !IS_IN_STASIS(src)) // REDMOON EDIT - after_death_stats_fix - добавлена проверка на стазис (нужна в начале раунда, чтобы не было усталости при спавне)
	...
	if(todd == "night")
		...
		if(HAS_TRAIT(src, TRAIT_NOSLEEP))
			return ..()
		// REDMOON ADD START - after_death_stats_fix
		if(IS_IN_STASIS(src))
			return ..()
		// REDMOON ADD END
		apply_status_effect(/datum/status_effect/debuff/sleepytime)
