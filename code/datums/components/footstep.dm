...
/datum/component/footstep/proc/play_simplestep()
	...
	if(!footstep_sounds[turf_footstep] || (LAZYLEN(footstep_sounds) < 3))
		testing("SOME MUPPET GAVE AN INVALID FOOTSTEP [footstep_type] VALUE ([turf_footstep]) TO [T.type]!!! FIX THIS SHIT!!!")
		return
...
/datum/component/footstep/proc/play_humanstep()
	...
	if(H.shoes || feetCover) //are we wearing shoes
		...
		if(!GLOB.footstep[T.footstep] || (LAZYLEN(GLOB.footstep[T.footstep]) < 3))
			testing("SOME MUPPET GAVE AN INVALID FOOTSTEP VALUE ([T.footstep]) TO [T.type]!!! FIX THIS SHIT!!!")
			return
