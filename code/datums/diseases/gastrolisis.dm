	...
	switch(stage)
		...
		if(4)
			...
			if(!eyes && prob(5))
				...
				"<span class='danger'>I scream in pain as your eyes are pushed out by your new snail eyes!</span>")
			...
			if(!shell && prob(5))
				if(affected_mob.dropItemToGround(affected_mob.get_item_by_slot(SLOT_BACK)))
					...
					"<span class='danger'>I scream in pain as a shell pushes itself out from under your skin!</span>")
					affected_mob.emote("scream")
					return
			...
			if(!tongue && prob(5))
				...
				to_chat(affected_mob, "<span class='danger'>I feel your speech slow down...</span>")
				return
			if(shell && eyes && tongue && prob(5))
				...
				"<span class='boldnotice'>I turned into a snail person! You feel an urge to cccrrraaawwwlll...</span>")
				cure()
			if(prob(10))
				affected_mob.emote("gag")
			if(prob(10))
...
/datum/disease/gastrolosis/cure()
	...
	if(!is_species(affected_mob, /datum/species/snail)) //undo all the snail fuckening
		...
		if(tongue)
			var/tongue_type = H.dna.species.organs[ORGAN_SLOT_TONGUE]
			var/obj/item/organ/tongue/new_tongue = new tongue_type()
			new_tongue.Insert(H)
		var/obj/item/organ/eyes/eyes = locate(/obj/item/organ/eyes/snail) in H.internal_organs
		if(eyes)
			var/eyes_typepath = H.dna.species.organs[ORGAN_SLOT_EYES]
			var/obj/item/organ/eyes/new_eyes = new eyes_typepath()
			new_eyes.Insert(H)
