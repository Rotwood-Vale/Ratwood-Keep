	...
	switch(stage)
		...
		if(4)
			...
			if(!eyes && prob(5))
				...
				span_danger("I scream in pain as your eyes are pushed out by your new snail eyes!"))
			...
			if(!shell && prob(5))
				if(affected_mob.dropItemToGround(affected_mob.get_item_by_slot(SLOT_BACK)))
					...
					span_danger("I scream in pain as a shell pushes itself out from under your skin!"))
					affected_mob.emote("scream")
					return
			...
			if(!tongue && prob(5))
				...
				to_chat(affected_mob, span_danger("I feel your speech slow down..."))
				return
			if(shell && eyes && tongue && prob(5))
				...
				span_boldnotice("I turned into a snail person! You feel an urge to cccrrraaawwwlll..."))
				cure()
			if(prob(10))
				affected_mob.emote("gag")
			if(prob(10))
