...
/datum/outfit/job/roguetown/heartfelt/magos/pre_equip(mob/living/carbon/human/H)
	...
	if(H.mind)
		...
		var/list/spells = list(/obj/effect/proc_holder/spell/targeted/touch/prestidigitation, /obj/effect/proc_holder/spell/invoked/projectile/fetch, /obj/effect/proc_holder/spell/invoked/projectile/lightningbolt, /obj/effect/proc_holder/spell/invoked/projectile/fireball)
		for(var/S in spells)
...
/datum/outfit/job/roguetown/heartfelt/prior/pre_equip(mob/living/carbon/human/H)
	...
	while(H.mind.used_spell_points < H.mind.spell_points)
		...
		switch(magichoice)
			...
			if("End Spell Selection (You will lose all points not spent)") // In case someone doesn't want to spend every point they have, for whatever reason
