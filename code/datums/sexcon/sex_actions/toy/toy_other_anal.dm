...
/datum/sex_action/toy_other_anal/New()
	. = ..()
	if(usr?.client?.prefs?.be_russian)
		name = "Попа. Использовать игрушку."
...
/datum/sex_action/toy_other_anal/on_start(mob/living/carbon/human/user, mob/living/carbon/human/target)
	...
	if(HAS_TRAIT(target, TRAIT_TINY) && !(HAS_TRAIT(user, TRAIT_TINY)))
		...
		if(user.client.prefs.be_russian)
			user.visible_message(span_warning("[user] вталкивает \the [dildo] в маленькую задницу [target]!"))
		else
		...
		if(user.client.prefs.be_russian)
			user.visible_message(span_warning("[user] вводит \the [dildo] в задницу [target]..."))
		else
...
/datum/sex_action/toy_other_anal/on_perform(mob/living/carbon/human/user, mob/living/carbon/human/target)
	...
	if(user.sexcon.do_message_signature("[type]"))
		if(HAS_TRAIT(target, TRAIT_TINY) && !(HAS_TRAIT(user, TRAIT_TINY)))
			if(user.client.prefs.be_russian)
				user.visible_message(user.sexcon.spanify_force("[user] [user.sexcon.get_generic_force_adjective()] трахает маленькую задницу [target] при помощи \the [dildo]..."))
			else
				user.visible_message(user.sexcon.spanify_force("[user] [user.sexcon.get_generic_force_adjective()] stuffs [target]'s tiny butt..."))
		else
			if(user.client.prefs.be_russian)
				user.visible_message(user.sexcon.spanify_force("[user] [user.sexcon.get_generic_force_adjective()] трахает задницу [target] при помощи \the [dildo]..."))
			else
...
/datum/sex_action/toy_other_anal/on_finish(mob/living/carbon/human/user, mob/living/carbon/human/target)
	...
	var/obj/item/dildo = get_dildo_in_either_hand(user)
	if(user.client.prefs.be_russian)
		user.visible_message(span_warning("[user] вытаскивает \the [dildo] из задницы [target]."))
	else
