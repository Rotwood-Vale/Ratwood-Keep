...
/datum/sex_controller/proc/show_ui()
	...
	if(!user.getorganslot(ORGAN_SLOT_PENIS))
		...
		dat += "<center><a href='byond://?src=[REF(src)];task=speed_down'>\<</a> [speed_name] <a href='byond://?src=[REF(src)];task=speed_up'>\></a> ~|~ <a href='byond://?src=[REF(src)];task=force_down'>\<</a> [force_name] <a href='byond://?src=[REF(src)];task=force_up'>\></a> ~|~ <a href='byond://?src=[REF(src)];task=manual_arousal_down'>\<</a> [manual_arousal_name] <a href='byond://?src=[REF(src)];task=manual_arousal_up'>\></a></center>"
	if(usr?.client?.prefs?.be_russian)
		dat += "<center>| <a href='byond://?src=[REF(src)];task=toggle_finished'>[do_until_finished ? "ДО ТЕХ ПОР, ПОКА Я НЕ ОСТАНОВЛЮСЬ" : "ДО ТЕХ ПОР, ПОКА Я НЕ ОСТАНОВЛЮСЬ"]</a> |</center>"
	else
		dat += "<center>| <a href='byond://?src=[REF(src)];task=toggle_finished'>[do_until_finished ? "UNTIL IM FINISHED" : "UNTIL I STOP"]</a> |</center>"
	if(usr?.client?.prefs?.be_russian)
		dat += "<center><a href='byond://?src=[REF(src)];task=set_arousal'>SET AROUSAL</a> | <a href='byond://?src=[REF(src)];task=freeze_arousal'>[arousal_frozen ? "НЕ СДЕРЖИВАТЬСЯ" : "СДЕРЖИВАТЬСЯ"]</a></center>"
	else
	...
	if(target == user)
		...
		dat += "<center>| <a href='byond://?src=[REF(src)];task=toggle_finished'>[do_until_finished ? "UNTIL I STOP" : "UNTIL I STOP"]</a> |</center>"
	if(target == user)
		if(usr?.client?.prefs?.be_russian)
	...
	if(current_action)
		if(usr?.client?.prefs?.be_russian)
			dat += "<center><a href='byond://?src=[REF(src)];task=stop'>Остановить</a></center>"
		else
	...
	for(var/action_type in GLOB.sex_actions)
		...
		if(current_action == action_type)
			link = "linkOn"
		if(usr?.client?.prefs?.be_russian)
			dat += "<center><a class='[link]' href='byond://?src=[REF(src)];task=action;action_type=[action_type]'>[action.ru_name]</a></center>"
		else
