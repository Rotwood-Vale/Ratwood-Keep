...
/datum/game_mode/chaosmode/after_DO()
	...
	switch(num_players(count_ready_only = TRUE))
		if(64 to INFINITY)
			var/major_roll_highpop = rand(1, 3)
			switch(major_roll_highpop)
				...
				if(3)
					...
					log_game("Major Antagonist: Lich")
		if(52 to 64)
			var/major_roll_midpop = rand(1, 2)
			...
			var/major_roll_lowpop = rand(1, 3)
...
/datum/game_mode/chaosmode/proc/pick_bandits()
	...
	var/num_bandits = 0
	var/limit_bandits = 8
	var/num_readied = num_players(count_ready_only = TRUE)
	if(num_readied >= 9)
		// 1 bandit per 9 players readied, up to 8
		num_bandits = clamp(round(num_readied / 9), 0, limit_bandits)
...
/datum/game_mode/chaosmode/proc/pick_rebels()
	...
	var/num_readied = num_players(count_ready_only = TRUE)
	if(num_readied >= 10)
		num_rebels = CLAMP(round(num_readied / 3), 1, 3)
	if(num_rebels)
