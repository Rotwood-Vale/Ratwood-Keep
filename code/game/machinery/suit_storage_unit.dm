...
/obj/machinery/suit_storage_unit/MouseDrop_T(atom/A, mob/living/user)
	...
	if(target == user)
		...
		target.visible_message(span_warning("[user] starts shoving [target] into [src]!"), span_danger("[user] starts shoving you into [src]!"))

	if(do_mob(user, target, 30))
		if(occupant || helmet || suit || storage)
			return
		if(target == user)
			user.visible_message(span_warning("[user] slips into [src] and closes the door behind [user.p_them()]!"), "<span class=notice'>I slip into [src]'s cramped space and shut its door.</span>")
		else
			target.visible_message(span_warning("[user] pushes [target] into [src] and shuts its door!"), span_danger("[user] shoves you into [src] and shuts the door!"))
...
/obj/machinery/suit_storage_unit/container_resist(mob/living/user)
	...
	user.visible_message(span_notice("I see [user] kicking against the doors of [src]!"), \
		...
		span_hear("I hear a thump from [src]."))
	...
	switch(action)
		...
		if("uv")
			if(occupant && safeties)
				...
				if(occupant)
					var/mob/living/mob_occupant = occupant
					to_chat(mob_occupant, span_danger("[src]'s confines grow warm, then hot, then scorching. You're being burned [!mob_occupant.stat ? "alive" : "away"]!"))
				cook()
				. = TRUE
		if("dispense")
			if(!state_open)
