...
/obj/item/extinguisher/AltClick(mob/user)
	...
	if(!user.is_holding(src))
		to_chat(user, span_notice("I must be holding the [src] in your hands do this!"))
		return
...
/obj/item/extinguisher/proc/EmptyExtinguisher(mob/user)
	if(loc == user && reagents.total_volume)
		...
		user.visible_message(span_notice("[user] empties out \the [src] onto the floor using the release valve."), span_info("I quietly empty out \the [src] using its release valve."))
