...
/obj/item/pneumatic_cannon/proc/Fire(mob/living/user, atom/target)
	...
	if(HAS_TRAIT(user, TRAIT_CLUMSY) && prob(75) && clumsyCheck && iscarbon(user))
		...
		C.visible_message(span_warning("[C] loses [C.p_their()] grip on [src], causing it to go off!"), span_danger("[src] slips out of your hands and goes off!"))
	...
	if(pressureSetting >= 3 && iscarbon(user))
		var/mob/living/carbon/C = user
		C.visible_message(span_warning("[C] is thrown down by the force of the cannon!"), "<span class='danger'>[src] slams into your shoulder, knocking you down!")
		C.Paralyze(60)
