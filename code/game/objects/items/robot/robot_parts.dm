...
/obj/item/robot_suit/attackby(obj/item/W, mob/user, params)
	...
	if(istype(W, /obj/item/stack/sheet/metal))
		...
		if(check_completion())
			...
			if(QDELETED(src) || QDELETED(BM) || QDELETED(user) || QDELETED(M) || !Adjacent(user))
				if(!QDELETED(M))
			...
			if(O.mind && O.mind.special_role)
				...
				to_chat(O, span_danger("I have been robotized!"))
...
/obj/item/robot_suit/Topic(href, href_list)
	...
	if(href_list["Name"])
		...
		if(!forced_ai)
			to_chat(usr, span_alert("No active AIs detected."))
