/obj/item/gun/ballistic/flintgonne
	name = "flintgonne"
	...
	gripped_intents = list(/datum/intent/shoot/flintgonne, /datum/intent/arc/flintgonne, INTENT_GENERIC)
...
/obj/item/gun/ballistic/flintgonne/getonmobprop(tag)
	. = ..()
	if(tag)
		switch(tag)
			if("gen")
...
/obj/item/gun/ballistic/flintgonne/Initialize()
	. = ..()
	myrod = new /obj/item/ramrod(src)


/obj/item/gun/ballistic/flintgonne/shoot_live_shot(mob/living/user as mob|obj, pointblank = 0, mob/pbtarget = null, message = 1)
...
/obj/item/gun/ballistic/flintgonne/attack_right(mob/user)
...
/obj/item/gun/ballistic/flintgonne/shoot_with_empty_chamber()
	playsound(src.loc, 'sound/foley/musketcock.ogg', 100, FALSE)
	update_icon()

/obj/item/gun/ballistic/flintgonne/attack_self(mob/living/user)
...
/datum/intent/shoot/flintgonne/get_chargetime()
...
/datum/intent/arc/flintgonne/can_charge()
	if(mastermob && masteritem.wielded)
		if(!masteritem.wielded)
			return FALSE
...
/obj/item/gun/ballistic/flintgonne/process_fire(atom/target, mob/living/user, message = TRUE, params = null, zone_override = "", bonus_spread = 0)
...
/obj/item/gun/ballistic/flintgonne/afterattack(atom/target, mob/living/user, flag, params)
	. = ..()
/*	if(!reloaded)
		to_chat(user, span_warning("The [src] is not properly loaded yet!"))
		return*/

/obj/item/gun/ballistic/flintgonne/can_shoot()
	if (!reloaded)
		return FALSE
	return ..()
