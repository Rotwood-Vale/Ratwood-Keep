...
/obj/item/book/granter/spellbook
	...
	"Didn’t I just read this page...?", "A lone illustration of Noc’s visage fills this page, his stony gaze boring into my soul...", \
...
/obj/item/book/granter/spellbook/on_reading_finished(mob/user)
	...
	var/qualityoflearn = (reader.STAINT*2 + (user.get_skill_level(/datum/skill/misc/reading)*10) + (user.get_skill_level(/datum/skill/magic/arcane)*5))
	...
	if(prob(55))
		...
		user.add_sleep_experience(/datum/skill/misc/reading, reader.STAINT*10)
...
/obj/item/natural/hide/attackby(obj/item/P, mob/living/carbon/human/user, params)
	...
	if(istype(P, /obj/item/paper/scroll))
		if(isturf(loc)&& (found_table))
			var/crafttime = 10 SECONDS - (user.get_skill_level(/datum/skill/magic/arcane) * 0.5 SECONDS)
...
/obj/item/spellbook_unfinished/pre_arcyne/attackby(obj/item/P, mob/living/carbon/human/user, params)
	...
	if(istype(P, /obj/item/roguegem))
		...
		if (the_rock.magic_power)
			if(isturf(loc) && (found_table))
				var/crafttime = (13 SECONDS - the_rock.magic_power) - (user.get_skill_level(/datum/skill/magic/arcane) * 0.5 SECONDS)
				if(do_after(user, crafttime, target = src))
					if (isarcyne(user))
						playsound(loc, 'sound/magic/crystal.ogg', 100, TRUE)
						user.visible_message(span_warning("[user] crushes [user.p_their()] [P]! Its powder seeps into the [src]."), \
