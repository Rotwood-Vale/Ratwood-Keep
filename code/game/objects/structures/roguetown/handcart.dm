...
/obj/item/cart_upgrade/level_1
	...
	icon_state = "upgrade"
	ulevel = 1
	var/filter/shadow_filter

	Initialize()
		. = ..()
		shadow_filter = filter(type="drop_shadow", x=0, y=0, size=0.5, offset=1, color=rgb(26, 13, 150, 150))
		filters = list(shadow_filter)
...
/obj/item/cart_upgrade/level_2
	...
	Initialize()
		. = ..()
		shadow_filter = filter(type="drop_shadow", x=0, y=0, size=0.5, offset=1, color=rgb(32, 196, 218, 200))
		filters = list(shadow_filter)
