...
/obj/structure/safe/proc/check_unlocked(mob/user, canhear)
	...
	if(user && canhear)
		if(tumbler_1_pos == tumbler_1_open)
			to_chat(user, span_hear("I hear a [pick("tonk", "krunk", "plunk")] from [src]."))
		if(tumbler_2_pos == tumbler_2_open)
			to_chat(user, span_hear("I hear a [pick("tink", "krink", "plink")] from [src]."))
...
/obj/structure/safe/Topic(href, href_list)
	...
	if(href_list["decrement"])
		...
		if(dial == tumbler_1_pos + 1 || dial == tumbler_1_pos - 71)
			...
			if(canhear)
				to_chat(user, span_hear("I hear a [pick("clack", "scrape", "clank")] from [src]."))
			if(tumbler_1_pos == tumbler_2_pos + 37 || tumbler_1_pos == tumbler_2_pos - 35)
				tumbler_2_pos = decrement(tumbler_2_pos)
				if(canhear)
					to_chat(user, span_hear("I hear a [pick("click", "chink", "clink")] from [src]."))
