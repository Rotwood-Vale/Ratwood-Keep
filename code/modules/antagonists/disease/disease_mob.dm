...
/mob/camera/disease/Hear(message, atom/movable/speaker, message_language, raw_message, radio_freq, list/spans, message_mode, original_message)
	. = ..()
...
/mob/camera/disease/proc/pick_name()
	...
	while(!set_name)
		...
		if(taken_names[input])
			to_chat(src, span_warning("I cannot use the name of such a well-known disease!"))
		else
			set_name = input
...
/mob/camera/disease/proc/infect_random_patient_zero(del_on_fail = TRUE)
	...
	if(del_on_fail)
		to_chat(src, span_danger("No hosts were available for your disease to infect."))
		qdel(src)
	return FALSE
...
/mob/camera/disease/proc/notify_adapt_ready()
	to_chat(src, span_notice("I are now ready to adapt again."))
	refresh_adaptation_menu()
