...
/mob/living/simple_animal/revenant/proc/Harvest(mob/living/carbon/human/target)
	if(!castcheck(0))
		return
	if(draining)
		to_chat(src, span_revenwarning("I are already siphoning the essence of a soul!"))
		return
	if(!target.stat)
		to_chat(src, span_revennotice("[target.p_their(TRUE)] soul is too strong to harvest."))
		if(prob(10))
			to_chat(target, span_revennotice("I feel as if you are being watched."))
		return
	face_atom(target)
	draining = TRUE
	...
	if(do_after(src, rand(10, 20), 0, target)) //did they get deleted in that second?
		...
		if(do_after(src, rand(15, 20), 0, target)) //did they get deleted NOW?
			...
			if(do_after(src, rand(15, 25), 0, target)) //how about now
				...
				to_chat(src, span_revenwarning("I are not close enough to siphon [target ? "[target]'s":"[target.p_their()]"] soul. The link has been broken."))
	draining = FALSE
	essence_drained = 0
		...
		if(iscarbon(mob))
			if(ishuman(mob))
				...
				if(!blightfound)
					...
					to_chat(H, span_revenminor("I feel [pick("suddenly sick", "a surge of nausea", "like your skin is <i>wrong</i>")]."))
			else
				if(mob.reagents)
