...
/datum/antagonist/maniac/process()
	if(!owner.current || triumphed)
		...
		return
	handle_visions(owner.current, hallucinations)
	if(waking_up)
...
/proc/handle_visions(mob/living/target, atom/movable/screen/fullscreen/maniac/hallucinations)
	if(prob(4))
...
/proc/handle_hallucinations(mob/living/target)
	//Chasing mob
	if(prob(1))
		INVOKE_ASYNC(target, GLOBAL_PROC_REF(handle_mob_hallucination), target)
	//Talking objects
	else if(prob(4))
		INVOKE_ASYNC(target, GLOBAL_PROC_REF(handle_object_hallucination), target)

/proc/handle_object_hallucination(mob/living/target)
...
/proc/handle_mob_hallucination(mob/living/target)
	if(!target.client)
		return
...
/proc/handle_floors(mob/living/target)
	if(!target.client)
		return
	...
	for(var/turf/open/floor in view(target))
		if(!prob(7))
			continue
		INVOKE_ASYNC(target, GLOBAL_PROC_REF(handle_floor), floor, target)

/proc/handle_floor(turf/open/floor, mob/living/target)
...
/proc/handle_walls(mob/living/target)
	if(!target.client)
		return
	//Shit on THA walls
	for(var/turf/closed/wall in view(target))
		if(!prob(4))
			continue
		INVOKE_ASYNC(target, GLOBAL_PROC_REF(handle_wall), wall, target)

/proc/handle_wall(turf/closed/wall, mob/living/target)
