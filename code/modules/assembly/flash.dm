...
/obj/item/assembly/flash/proc/burn_out() //Made so you can override it if you want to have an invincible flash from R&D or something.
	...
	if(ismob(loc))
		var/mob/M = loc
		M.visible_message(span_danger("[src] burns out!"),span_danger("[src] burns out!"))
	else
...
/obj/item/assembly/flash/attack(mob/living/M, mob/user)
	...
	if(iscarbon(M))
		...
		user.visible_message(span_warning("[user] overloads [R]'s sensors with the flash!"), span_danger("I overload [R]'s sensors with the flash!"))
		return TRUE

	user.visible_message(span_warning("[user] fails to blind [M] with the flash!"), span_warning("I fail to blind [M] with the flash!"))
	...
	if(targeted)
		if(M.flash_act(1, 1))
			...
			if(M.hypnosis_vulnerable())
				hypnosis = TRUE
			if(user)
				user.visible_message(span_danger("[user] blinds [M] with the flash!"), span_danger("I hypno-flash [M]!"))
			...
			user.visible_message(span_warning("[user] fails to blind [M] with the flash!"), span_warning("I fail to hypno-flash [M]!"))
		else
			to_chat(M, span_danger("[src] fails to blind you!"))
