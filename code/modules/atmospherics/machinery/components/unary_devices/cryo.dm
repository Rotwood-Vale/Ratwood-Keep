...
/obj/machinery/atmospherics/components/unary/cryo_cell
	...
	density = TRUE
	max_integrity = 350
	armor = list("blunt" = 0, "slash" = 0, "stab" = 0, "bullet" = 0, "laser" = 0, "energy" = 100, "bomb" = 0, "bio" = 100, "rad" = 100, "fire" = 30, "acid" = 30)
...
/obj/machinery/atmospherics/components/unary/cryo_cell/process()
	...
	if(mob_occupant.health >= mob_occupant.getMaxHealth()) // Don't bother with fully healed people.
		on = FALSE
		update_icon()
		playsound(src, 'sound/blank.ogg', volume) // Bug the doctors.
...
/obj/machinery/atmospherics/components/unary/cryo_cell/MouseDrop_T(mob/target, mob/user)
	...
	if(isliving(target))
		...
		user.visible_message(span_notice("[user] starts shoving [target] inside [src]."), span_notice("I start shoving [target] inside [src]."))
...
/obj/machinery/atmospherics/components/unary/cryo_cell/attackby(obj/item/I, mob/user, params)
	...
	if(!on && !occupant && !state_open && (default_deconstruction_screwdriver(user, "pod-off", "pod-off", I)) \
		...
		to_chat(user, "<span class='warning'>I can't access the maintenance panel while the pod is " \
