...
/turf/open/lava/plasma/attackby(obj/item/I, mob/user, params)
	...
	user.visible_message(span_notice("[user] scoops some plasma from the [src] with \the [C]."), span_notice("I scoop out some plasma from the [src] using \the [C]."))
					...
					if(prob(35)) //checking if the delay is over & if the victim actually has any parts to nom
						...
						if(plasma_parts.len)
							...
							PP.visible_message(span_warning("[L] screams in pain as [L.p_their()] [NB] melts down to the bone!"), \
											  span_danger("I scream out in pain as my [NB] melts down to the bone, leaving an eerie plasma-like glow where flesh used to be!"))
						if(!plasma_parts.len && !robo_parts.len) //a person with no potential organic limbs left AND no robotic limbs, time to turn them into a plasmaman
							...
							PP.visible_message(span_warning("[L] bursts into a brilliant purple flame as [L.p_their()] entire body is that of a skeleton!"), \
											  span_danger("My senses numb as all of my remaining flesh is turned into a purple slurry, sloshing off my body and leaving only my bones to show in a vibrant purple!"))
...
/obj/item/clothing/under/syndicate/coldres
	...
	desc = ""
	armor = list("blunt" = 20, "slash" = 15, "stab" = 10, "bullet" = 10, "laser" = 0,"energy" = 5, "bomb" = 0, "bio" = 0, "rad" = 0, "fire" = 25, "acid" = 25)
