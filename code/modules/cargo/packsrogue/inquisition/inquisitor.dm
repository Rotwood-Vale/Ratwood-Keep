/datum/supply_pack/rogue/inquisition/Inquisitor
	group = "Inquisitor"
	crate_name = "Gifts of Inquisitor."
	crate_type = /obj/structure/closet/crate/chest/merchant

/datum/supply_pack/rogue/inquisition/Inquisitor/belt
	name = "Leather Belt"
	cost = 20
	contains = list(/obj/item/storage/belt/rogue/leather)

/datum/supply_pack/rogue/inquisition/Inquisitor/leatherpants
	name = "Leather trousers"
	cost = 40
	contains = list(/obj/item/clothing/under/roguetown/trou/leather)

/datum/supply_pack/rogue/inquisition/Inquisitor/leathergloves
	name = "Leather gloves"
	cost = 20
	contains = list(/obj/item/clothing/gloves/roguetown/leather)

/datum/supply_pack/rogue/inquisition/Inquisitor/leathershoes
	name = "Leather shoes"
	cost = 20
	contains = list(/obj/item/clothing/shoes/roguetown/armor/leather)

/datum/supply_pack/rogue/inquisition/Inquisitor/hardleathershoes
	name = "Hardened Leather shoes"
	cost = 60
	contains = list(/obj/item/clothing/shoes/roguetown/armor/leather/advanced)

/datum/supply_pack/rogue/inquisition/Inquisitor/hood
	name = "Shoulder Hood"
	cost = 20
	contains = list(/obj/item/clothing/head/roguetown/roguehood/random)

/datum/supply_pack/rogue/inquisition/Inquisitor/leather/Lhelmet
	name = "Reinforced hood"
	cost = 100
	contains = list(/obj/item/clothing/head/roguetown/roguehood/reinforced)

/datum/supply_pack/rogue/inquisition/Inquisitor/undershirt
	name = "Undershirt"
	cost = 20
	contains = list(/obj/item/clothing/suit/roguetown/shirt/undershirt/random)

/datum/supply_pack/rogue/inquisition/Inquisitor/light_gambeson
	name = "Light Gambeson"
	cost = 30
	contains = list(/obj/item/clothing/suit/roguetown/armor/gambeson/light)

/datum/supply_pack/rogue/inquisition/Inquisitor/tights
	name = "Tights"
	cost = 20
	contains = list(/obj/item/clothing/under/roguetown/tights/random)

/datum/supply_pack/rogue/inquisition/Inquisitor/shortshirt
	name = "Shortshirt"
	cost = 20
	contains = list(/obj/item/clothing/suit/roguetown/shirt/shortshirt/random)

/datum/supply_pack/rogue/inquisition/Inquisitor/halfcloak
	name = "Half-Cloaks"
	cost = 20
	contains = list(/obj/item/clothing/cloak/half)

/datum/supply_pack/rogue/inquisition/Inquisitor/graincloak
	name = "Green Rain-Cloak"
	cost = 25
	contains = list(/obj/item/clothing/cloak/raincloak/green)

/datum/supply_pack/rogue/inquisition/Inquisitor/braincloak
	name = "Blue Rain-Cloak"
	cost = 25
	contains = list(/obj/item/clothing/cloak/raincloak/blue)

/datum/supply_pack/rogue/inquisition/Inquisitor/rraincloak
	name = "Red Rain-Cloak"
	cost = 25
	contains = list(/obj/item/clothing/cloak/raincloak/red)

/datum/supply_pack/rogue/inquisition/Inquisitor/rraincloak
	name = "Fur Cloak"
	cost = 60
	contains = list(/obj/item/clothing/cloak/raincloak/furcloak)

/datum/supply_pack/rogue/inquisition/Inquisitor/cape
	name = "Black Cape"
	cost = 30
	contains = list(/obj/item/clothing/cloak/cape/black)

/datum/supply_pack/rogue/inquisition/Inquisitor/tabard
	name = "Tabard"
	cost = 30
	contains = list(/obj/item/clothing/cloak/tabard)

/datum/supply_pack/rogue/inquisition/Inquisitor/hidearmor
	name = "Hide armor"
	cost = 40
	contains = list(/obj/item/clothing/suit/roguetown/armor/leather/hide)

/datum/supply_pack/rogue/inquisition/Inquisitor/halfmasks
	name = "Halfmask"
	cost = 80
	contains = list(/obj/item/clothing/mask/rogue/shepherd)

/datum/supply_pack/rogue/inquisition/Inquisitor/masks
	name = "Steel Mask"
	cost = 80
	contains = list(/obj/item/clothing/mask/rogue/facemask/steel)

/datum/supply_pack/rogue/inquisition/Inquisitor/maskhound
	name = "Hound Mask"
	cost = 80
	contains = list(/obj/item/clothing/mask/rogue/facemask/steel/hound)

/datum/supply_pack/rogue/inquisition/Inquisitor/maskgold
	name = "Gold Mask"
	cost = 350
	contains = list(/obj/item/clothing/mask/rogue/facemask/goldmask)

/datum/supply_pack/rogue/inquisition/Inquisitor/coif
	name = "Coif"
	cost = 40
	contains = list(/obj/item/clothing/neck/roguetown/coif)

/datum/supply_pack/rogue/inquisition/Inquisitor/foodgood
	name = "Frybird & Tato"
	cost = 80
	contains = list(/obj/item/reagent_containers/food/snacks/rogue/frybirdtato/plated)

/datum/supply_pack/rogue/inquisition/Inquisitor/foodbad
	name = "Dry Meat"
	cost = 20
	contains = list(/obj/item/reagent_containers/food/snacks/rogue/meat/coppiette)

/datum/supply_pack/rogue/inquisition/Inquisitor/wine
	name = "Wine"
	cost = 100
	contains = list(/obj/item/reagent_containers/glass/bottle/rogue/wine)

/datum/supply_pack/rogue/inquisition/Inquisitor/ropeleash
	name = "Rope Leash"
	cost = 5
	contains = list(/obj/item/leash)

/datum/supply_pack/rogue/inquisition/Inquisitor/leatherleash
	name = "Leather Leash"
	cost = 20
	contains = list(/obj/item/leash/leather)

/datum/supply_pack/rogue/inquisition/Inquisitor/chainleash
	name = "Chain Leash"
	cost = 50
	contains = list(/obj/item/leash/chain)

/datum/supply_pack/rogue/inquisition/Inquisitor/leathercollar
	name = "Leather Collar"
	cost = 20
	contains = list(/obj/item/clothing/neck/roguetown/collar/leather)

/datum/supply_pack/rogue/inquisition/Inquisitor/collarcat
	name = "Leather Cat Collar"
	cost = 40
	contains = list(/obj/item/clothing/neck/roguetown/collar/leather/bell)

/datum/supply_pack/rogue/inquisition/Inquisitor/collarcow
	name = "Leather Cow Collar"
	cost = 40
	contains = list(/obj/item/clothing/neck/roguetown/collar/leather/bell/cow)
