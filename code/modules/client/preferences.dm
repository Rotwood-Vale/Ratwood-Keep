	...
	if(user)
		if(usr?.client?.prefs?.be_russian)
			...
			if(pref_species.ru_expanded_desc)
				to_chat(user, "<a href='byond://?src=[REF(user)];view_species_info=[pref_species.ru_expanded_desc]'>Читать ещё</a>")
		else
			if(pref_species.desc)
				to_chat(user, "[pref_species.desc]")
			if(pref_species.expanded_desc)
				to_chat(user, "<a href='byond://?src=[REF(user)];view_species_info=[pref_species.expanded_desc]'>Read More</a>")
		if(usr?.client?.prefs?.be_russian)
