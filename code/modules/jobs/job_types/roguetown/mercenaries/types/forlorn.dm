...
/datum/outfit/job/roguetown/mercenary/forlorn/pre_equip(mob/living/carbon/human/H)
	...
	switch(weaponschoice)
		...
		if("axe")
			...
			backl = /obj/item/rogueweapon/shield/heater
			H.adjust_skillrank(/datum/skill/combat/axes, 4, TRUE) // This brings the axe class in line-
			H.adjust_skillrank(/datum/skill/combat/maces, -1, TRUE) //
			H.adjust_skillrank(/datum/skill/combat/swords, -2, TRUE) // -for the same total skill points overall
		if("crossbow")
			beltl = /obj/item/ammo_holder/quiver/bolts
			backl = /obj/item/gun/ballistic/revolver/grenadelauncher/crossbow
			H.adjust_skillrank(/datum/skill/combat/crossbows, 2, TRUE) // Takes the crossbow merc to expert crossbows-
			H.adjust_skillrank(/datum/skill/combat/bows, 2, TRUE) // -and competent bows overall, as all mercs are-
			H.adjust_skillrank(/datum/skill/combat/maces, -2, TRUE) // -experts with their respective weapons.
			H.adjust_skillrank(/datum/skill/combat/swords, -1, TRUE) // Apprentice swordsmen. 
			backpack_contents = list(/obj/item/rogueweapon/huntingknife) // They need something to take heads with
		else
			beltl = /obj/item/rogueweapon/mace/warhammer/steel
			backl = /obj/item/rogueweapon/shield/heater
