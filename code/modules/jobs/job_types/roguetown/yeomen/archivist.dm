	...
	if(isliving(targets[1]))
		...
		if(L == usr)
			...
			if(L in range(2, usr))
				...
				if(!(item in list(/datum/skill/misc/reading, /datum/skill/misc/music, /datum/skill/craft/cooking, /datum/skill/misc/music, /datum/skill/misc/sewing)) && L.get_skill_level(item) < SKILL_LEVEL_NOVICE)
