...
/proc/get_multiz_accessible_levels(center_z)
	...
	var/offset
	while((offset = SSmapping.multiz_levels[other_z][Z_LEVEL_DOWN])) // REDMOON EDIT - multi-Z_fix - WAS: while((offset = SSmapping.multiz_levels[other_z]["[DOWN]"]))
		other_z -= offset
		if(other_z in .)
			break	// no infinite loops
		. += other_z
	other_z = center_z
	while((offset = SSmapping.multiz_levels[other_z][Z_LEVEL_UP])) // REDMOON EDIT - multi-Z_fix - WAS: while((offset = SSmapping.multiz_levels[other_z]["[UP]"]))
		other_z += offset
		if(other_z in .)
			break	// no infinite loops
		. += other_z
