...
/obj/item/extraction_pack/attack_self(mob/user)
	...
	if(!possible_beacons.len)
		...
		if(!A)
			return
		beacon = A
		to_chat(user, span_notice("I link the extraction pack to the beacon system."))
...
/obj/item/extraction_pack/afterattack(atom/movable/A, mob/living/carbon/human/user, flag, params)
	...
	if(!istype(A))
		...
		if(A.anchored || (A.move_resist > max_force_fulton))
			return
		to_chat(user, span_notice("I start attaching the pack to [A]..."))
		if(do_after(user,50,target=A))
			to_chat(user, span_notice("I attach the pack to [A] and activate it."))
			...
			if(isliving(A))
				...
				M.buckled = null // Unbuckle them to prevent anchoring problems
			else
				A.anchored = TRUE
				A.density = FALSE
			...
			playsound(holder_obj.loc, 'sound/blank.ogg', 50, TRUE, -3)
