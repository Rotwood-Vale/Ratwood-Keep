...
/*			if(VVictim)
				to_chat(user, span_notice("A strange, sweet taste tickles my throat."))
	...
	C.visible_message(span_danger("[user] drinks from [C]'s [sublimb_grabbed]!"), \
					span_userdanger("[user] drinks from my [sublimb_grabbed]!"), span_hear("..."), COMBAT_MESSAGE_RANGE, user)
	to_chat(user, span_warning("I drink from [C]'s [sublimb_grabbed]."))
