	...
	if((L.mobility_flags & MOBILITY_STAND) && (mobility_flags & MOBILITY_STAND)) //we are both standing
		if(I)
			if(I.wlength > WLENGTH_NORMAL)
				CZ = TRUE
			else if(HAS_TRAIT(src, TRAIT_MIDGET)) //midget variant, allows neck no head
			...
			else if(!HAS_TRAIT(src, TRAIT_MIDGET)) //we have a short/medium weapon, so allow hitting legs
			...
			if(HAS_TRAIT(src, TRAIT_MIDGET) && (!CZ)) //midget variant punching, no head but yes legs
			...
			else if(!HAS_TRAIT(src, TRAIT_MIDGET) && (!CZ)) //we are punching, no legs
