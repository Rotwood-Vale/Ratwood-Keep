...
/mob/living/CanPass(atom/movable/mover, turf/target)
	...
	if(ismob(mover))
		...
		if(isliving(mover))
			...
			if(M.wallpressed)
				return !wallpressed
	return (!density || wallpressed || (pixelshift_x >= 10 && pixel_x >= 10) || (pixelshift_x <= -10 && pixel_x <= -10) || (pixelshift_y >= 10 && pixel_y >= 10) || (pixelshift_y <= -10 && pixel_y <= -10) || !(mobility_flags & MOBILITY_STAND))
