				...
				if(do_parry(used_weapon, drained, user)) //show message
					...
					if(U.mind)
						if((U.mobility_flags & MOBILITY_STAND) && defender_skill && (attacker_skill < defender_skill - SKILL_LEVEL_NOVICE))
							if(AB)
								U.mind.add_sleep_experience(AB.associated_skill, max(round(U.STAINT/2), 0), FALSE)
							else
								U.mind.add_sleep_experience(/datum/skill/combat/unarmed, max(round(STAINT/2), 0), FALSE)
