/mob/living/simple_animal/bot/honkbot
	...
	var/honksound = 'sound/blank.ogg' //customizable sound
	var/spam_flag = FALSE
	var/cooldowntime = 30
...
/mob/living/simple_animal/bot/honkbot/emag_act(mob/user)
	...
	if(emagged == 2)
		if(user)
			user << span_danger("I short out [src]'s sound control system. It gives out an evil laugh!!")
			oldtarget_name = user.name
		audible_message(span_danger("[src] gives out an evil laugh!"))
		playsound(src, 'sound/blank.ogg', 75, TRUE, -1) // evil laughter
		update_icon()
...
/mob/living/simple_animal/bot/honkbot/hitby(atom/movable/AM, skipcatch = FALSE, hitpush = TRUE, blocked = FALSE, datum/thrownthing/throwingdatum, d_type = "blunt")
...
/mob/living/simple_animal/bot/honkbot/proc/stun_attack(mob/living/carbon/C) // airhorn stun
	if(!spam_flag)
		playsound(src, 'sound/blank.ogg', 100, TRUE, -1) //HEEEEEEEEEEEENK!!
		sensor_blink()
	if(spam_flag == 0)
		if(ishuman(C))
			C.stuttering = 20
			...
			C.visible_message(span_danger("[src] has honked [C]!"),\
					span_danger("[src] has honked you!"))
		else
			C.stuttering = 20
			C.Paralyze(80)
			addtimer(CALLBACK(src, PROC_REF(spam_flag_false)), cooldowntime)
