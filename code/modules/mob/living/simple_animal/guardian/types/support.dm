...
/mob/living/simple_animal/hostile/guardian/healer
	...
	playstyle_string = span_holoparasite("As a <b>support</b> type, you may toggle my basic attacks to a healing mode. In addition, Alt-Clicking on an adjacent object or mob will warp them to my bluespace beacon after a short delay.")
...
/mob/living/simple_animal/hostile/guardian/healer/ToggleMode()
	if(src.loc == summoner)
		if(toggle)
			...
			to_chat(src, "<span class='danger'><B>I switch to healing mode.</span></B>")
			toggle = TRUE
	else
...
/mob/living/simple_animal/hostile/guardian/healer/verb/Beacon()
	...
	if(beacon_cooldown >= world.time)
		to_chat(src, "<span class='danger'><B>My power is on cooldown. You must wait five minutes between placing beacons.</span></B>")
		return
	...
	to_chat(src, "<span class='danger'><B>Beacon placed! You may now warp targets and objects to it, including my user, via Alt+Click.</span></B>")
...
/mob/living/simple_animal/hostile/guardian/healer/AltClickOn(atom/movable/A)
	if(!istype(A))
		return
	if(src.loc == summoner)
		to_chat(src, "<span class='danger'><B>I must be manifested to warp a target!</span></B>")
		return
	if(!beacon)
		to_chat(src, "<span class='danger'><B>I need a beacon placed to warp things!</span></B>")
		return
	if(!Adjacent(A))
		to_chat(src, "<span class='danger'><B>I must be adjacent to my target!</span></B>")
		return
	if(A.anchored)
		to_chat(src, "<span class='danger'><B>My target cannot be anchored!</span></B>")
		return
	...
	to_chat(src, "<span class='danger'><B>I begin to warp [A].</span></B>")
	A.visible_message(span_danger("[A] starts to glow faintly!"), \
	span_danger("I start to faintly glow, and you feel strangely weightless!"))
	do_attack_animation(A)

	if(!do_mob(src, A, 60)) //now start the channel
		to_chat(src, "<span class='danger'><B>I need to hold still!</span></B>")
		return
	...
	span_danger("My vision is obscured by a flash of light!"))
