...
/datum/action/innate/megafauna_attack/dash
	...
	chosen_message = span_colossus("I are now dashing to your target.")
	chosen_attack_num = 1
...
/datum/action/innate/megafauna_attack/kinetic_accelerator
	...
	chosen_message = span_colossus("I are now shooting your kinetic accelerator.")
	chosen_attack_num = 2
...
/mob/living/simple_animal/hostile/retaliate/rogue/megafauna/blood_drunk_miner/AttackingTarget()
	...
	if(isliving(target))
		...
		if(L.stat == DEAD)
			...
			span_danger("I butcher [L], restoring your health!"))
