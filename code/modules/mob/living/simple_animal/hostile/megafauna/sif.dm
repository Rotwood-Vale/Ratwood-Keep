...
/mob/living/simple_animal/hostile/retaliate/rogue/megafauna/sif
	name = "Fenrir, Howl of the Storm"
	desc = "Guardian of the abyss. Looks pretty pissed that you're here."

	movement_type = GROUND
	attack_verb_continuous = "slashes"
	attack_verb_simple = "slash"
	attack_sound = 'modular_hearthstone/sound/sif/sif_slash.ogg'
	icon_state = "Great_Brown_Wolf"
	icon_living = "Great_Brown_Wolf"
	icon_dead = ""
	friendly_verb_continuous = "stares down"
	friendly_verb_simple = "stare down"
	icon = 'modular_hearthstone/icons/mob/lavaland/sif.dmi'
...
//	stat_attack = UNCONSCIOUS
	...
	deathsound = 'modular_hearthstone/sound/sif/death_howl.ogg'
...
/mob/living/simple_animal/hostile/retaliate/rogue/megafauna/sif/Initialize(mapload)
	. = ..()
	internal = new /obj/item/gps/internal/sif(src) */

//Sword structure, used to summon sif.
/obj/structure/sword/sif
	name = "Giant Frozen Godsword"
...
/obj/structure/sword/sif/attack_hand(mob/user)
	...
	playsound(get_turf(src.loc), 'modular_hearthstone/sound/effects/curse1.ogg', 100, 1)
	if(last_process + time_between_uses > world.time)
		to_chat(user, span_artery("The sword refuses to activate."))
		return
	last_process = world.time
	to_chat(user, span_hypnophrase("A long, distant howl creeps across the land."))
	playsound(src, pick('sound/vo/mobs/wwolf/howl (1).ogg','sound/vo/mobs/wwolf/howl (2).ogg'), 100, TRUE)
	spawn(50)
		if(!QDELETED(src))
			new /mob/living/simple_animal/hostile/retaliate/rogue/megafauna/sif(get_turf(src.loc))
			src.visible_message(span_hypnophrase("A massive beast leaps over the mountain, snatching the sword off the ground."))
			playsound(src, pick('sound/vo/mobs/wwolf/howl (1).ogg','sound/vo/mobs/wwolf/howl (2).ogg','sound/vo/mobs/vw/aggro (1).ogg','sound/vo/mobs/vw/aggro (2).ogg'), 100, TRUE)
			playsound(get_turf(src.loc), 'modular_hearthstone/sound/effects/curse3.ogg', 100, 1)
			playsound(get_turf(src.loc), 'modular_hearthstone/sound/effects/meteorimpact.ogg', 100, 1)
		//	qdel(src)
	update_icon()
	addtimer(CALLBACK(src, TYPE_PROC_REF(/atom, update_icon)), time_between_uses)
/obj/structure/sword/sif/update_icon()
	if(last_process + time_between_uses > world.time)
		name = "???" 
		icon_state = "Interact_Sword"
		desc = "Huh? It became spectral."
		alpha = 150
	else
		name = "Giant Frozen Godsword"
		icon_state = "Idle_Sword"
		...
		alpha = 255
//Sif's charge attack
/mob/living/simple_animal/hostile/retaliate/rogue/megafauna/sif/proc/rush()
...
/mob/living/simple_animal/hostile/retaliate/rogue/megafauna/sif/Move()
	//Move
	..()
...
/mob/living/simple_animal/hostile/retaliate/rogue/megafauna/sif/proc/spinAttack()
	src.spinning = TRUE
...
/mob/living/simple_animal/hostile/retaliate/rogue/megafauna/sif/bullet_act(obj/item/projectile/P)
	...
	if(passed == 1)
		...
		playsound(src, pick('modular_hearthstone/sound/weapons/bulletflyby.ogg', 'modular_hearthstone/sound/weapons/bulletflyby2.ogg', 'modular_hearthstone/sound/weapons/bulletflyby3.ogg'), 300, 1)
		return FALSE

	return ..()

//Sets Sif's angered stats
/mob/living/simple_animal/hostile/retaliate/rogue/megafauna/sif/proc/angered()
	src.angered = TRUE
	src.stageTwo = TRUE
	src.visible_message(span_userdanger("[src] lets out a ear ripping howl!"), span_userdanger("[src] lets out an ear ripping roar!"))
	playsound(src, 'modular_hearthstone/sound/sif/howl.ogg', 100, 1)
...
/mob/living/simple_animal/hostile/retaliate/rogue/megafauna/sif/proc/enraged()
	src.stageThree = TRUE
	src.enraged = TRUE
...
/mob/living/simple_animal/hostile/retaliate/rogue/megafauna/sif/proc/special()
	src.currentPower = 0
	switch(rand(1,2))
		if(1)
			rush()
...
/mob/living/simple_animal/hostile/retaliate/rogue/megafauna/sif/proc/default_attackspeed()
	if(stageTwo)
		src.move_to_delay = 2
		return 10
	if(stageThree)
...
/mob/living/simple_animal/hostile/retaliate/rogue/megafauna/sif/do_attack_animation(atom/A, visual_effect_icon,used_item, no_effect)
	if(charging == FALSE)
		..()

//Attack speed delay
//bob's note: everything here is well coded except this like why would you not just use changeNext_move()
/mob/living/simple_animal/hostile/retaliate/rogue/megafauna/sif/AttackingTarget()
	if(charging == FALSE)
		. = ..()
		if(.)
			recovery_time = world.time + 10

/mob/living/simple_animal/hostile/retaliate/rogue/megafauna/sif/Goto(target, delay, minimum_distance)
	if(charging == FALSE)
		..()

/mob/living/simple_animal/hostile/retaliate/rogue/megafauna/sif/MoveToTarget(list/possible_targets)
	if(charging == FALSE)
		..()

//Immune to explosions when spinning or charging
/mob/living/simple_animal/hostile/retaliate/rogue/megafauna/sif/ex_act(severity, target, origin)
	return FALSE

//stop spinning if you lose the target
/mob/living/simple_animal/hostile/retaliate/rogue/megafauna/sif/LoseTarget()
	. = ..()
	if(spinning)
		icon_state = "Great_Brown_Wolf"
		src.spinIntervals = 0
...
/mob/living/simple_animal/hostile/retaliate/rogue/megafauna/sif/Moved()
			...
			if(isturf(src.loc) || isobj(src.loc) && src.loc.density)
				...
				for(var/mob/living/LM in view(1, src))
					if(!(LM in hit_things))
						LM.Stun(30, TRUE)
						hit_things += LM
				playsound(src, pick('modular_hearthstone/sound/sif/whoosh1.ogg', 'modular_hearthstone/sound/sif/whoosh2.ogg', 'modular_hearthstone/sound/sif/whoosh3.ogg'), 300, 1)
				playsound(src, 'modular_hearthstone/sound/sif/blade_spin.ogg', 400, 1)
				if(angered)
					src.speed = 6
					src.move_to_delay = 2

	playsound(src, 'modular_hearthstone/sound/effects/meteorimpact.ogg', 200, 1, 2, 1)
	..()

//Activated when sif collides with target when charging.
/mob/living/simple_animal/hostile/retaliate/rogue/megafauna/sif/Bump(atom/A)
	if(charging == TRUE)
		...
		if(isliving(A))
			...
			playsound(get_turf(L), 'modular_hearthstone/sound/sif/sif_stomp.ogg', 400, 1)
			shake_camera(L, 4, 3)
