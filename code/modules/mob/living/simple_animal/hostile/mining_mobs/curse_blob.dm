/mob/living/simple_animal/hostile/retaliate/rogue/asteroid/curseblob
	name = "curse mass"
	desc = ""
...
/mob/living/simple_animal/hostile/retaliate/rogue/asteroid/curseblob/Initialize(mapload)
	. = ..()
	timerid = QDEL_IN(src, 600)
	playsound(src, 'sound/blank.ogg', 100, TRUE, -1)

/mob/living/simple_animal/hostile/retaliate/rogue/asteroid/curseblob/Destroy()
	new /obj/effect/temp_visual/dir_setting/curse/blob(loc, dir)
	doing_move_loop = FALSE
	return ..()

/mob/living/simple_animal/hostile/retaliate/rogue/asteroid/curseblob/Goto(move_target, delay, minimum_distance)
	move_loop(target, delay)

/mob/living/simple_animal/hostile/retaliate/rogue/asteroid/curseblob/proc/move_loop(move_target, delay)
	set waitfor = FALSE
	if(doing_move_loop)
		return
...
/mob/living/simple_animal/hostile/retaliate/rogue/asteroid/curseblob/proc/check_for_target()
	if(QDELETED(set_target) || set_target.stat != CONSCIOUS || z != set_target.z)
		qdel(src)
		return TRUE

/mob/living/simple_animal/hostile/retaliate/rogue/asteroid/curseblob/GiveTarget(new_target)
	if(check_for_target())
		return
...
/mob/living/simple_animal/hostile/retaliate/rogue/asteroid/curseblob/LoseTarget() //we can't lose our target!
	if(check_for_target())
		return

//if it's not our target, we ignore it
/mob/living/simple_animal/hostile/retaliate/rogue/asteroid/curseblob/CanPass(atom/movable/mover, turf/target)
	if(mover == set_target)
		return FALSE
...
#define IGNORE_PROC_IF_NOT_TARGET(X) /mob/living/simple_animal/hostile/retaliate/rogue/asteroid/curseblob/##X(AM) { if (AM == set_target) return ..(); }
...
/mob/living/simple_animal/hostile/retaliate/rogue/asteroid/curseblob/bullet_act(obj/projectile/Proj)
	if(Proj.firer != set_target)
		return
	return ..()

/mob/living/simple_animal/hostile/retaliate/rogue/asteroid/curseblob/attacked_by(obj/item/I, mob/living/L)
	if(L != set_target)
		return
	return ..()
