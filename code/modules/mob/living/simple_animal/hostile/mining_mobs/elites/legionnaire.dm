...
/**
  ...
  * - Leaves a pile of bones at my location.  Upon using this skill again, you'll swap locations with the bone pile.
...
/mob/living/simple_animal/hostile/asteroid/elite/legionnaire
	...
	attack_verb_simple = "slash my arms at"
...
/datum/action/innate/elite_attack/legionnaire_charge
	...
	chosen_message = span_boldwarning("I will attempt to grab my opponent and throw them.")
	chosen_attack_num = LEGIONNAIRE_CHARGE

/datum/action/innate/elite_attack/head_detach
	name = "Release Head"
	button_icon_state = "head_detach"
	chosen_message = span_boldwarning("I will now detach my head or kill it if it is already released.")
	chosen_attack_num = HEAD_DETACH

/datum/action/innate/elite_attack/bonfire_teleport
	name = "Bonfire Teleport"
	button_icon_state = "bonfire_teleport"
	chosen_message = span_boldwarning("I will leave a bonfire.  Second use will let you swap positions with it indefintiely.  Using this move on the same tile as my active bonfire removes it.")
...
/mob/living/simple_animal/hostile/asteroid/elite/legionnaire/proc/legionnaire_charge_2(move_dir, times_ran)
	...
	for(var/mob/living/L in T.contents - hit_things - src)
		...
		to_chat(L, span_danger("[src] grabs you and throws you with much force!"))
...
/mob/living/simple_animal/hostile/asteroid/elite/legionnaire/proc/bonfire_teleport()
	...
	if(mypile == null)
		...
		mypile = newpile
		mypile.myowner = src
		playsound(get_turf(src),'sound/blank.ogg', 200, 1)
		...
		if(legionturf == pileturf)
			...
			mypile = null
			return
		playsound(pileturf,'sound/blank.ogg', 200, 1)
		playsound(legionturf,'sound/blank.ogg', 200, 1)
