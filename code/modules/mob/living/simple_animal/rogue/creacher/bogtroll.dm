...
/mob/living/simple_animal/hostile/retaliate/rogue/bogtroll/taunted(mob/user)
	emote("aggro")
	Retaliate()
	GiveTarget(user)
	return

/mob/living/simple_animal/hostile/retaliate/bogtroll/Life()
	..()
	if(pulledby)
		Retaliate()
		GiveTarget(pulledby)

/mob/living/simple_animal/hostile/retaliate/rogue/bogtroll/get_sound(input)
	switch(input)
		if("aggro")
