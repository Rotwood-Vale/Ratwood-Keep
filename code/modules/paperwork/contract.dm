...
/obj/item/paper/contract/employment/attack(mob/living/M, mob/living/carbon/human/user)
	...
	if(deconvert)
		...
		to_chat(M, span_boldnotice("I feel that my soul has returned to its rightful owner, Nanotrasen."))
		M.return_soul()
	else
...
/obj/item/paper/contract/infernal/attackby(obj/item/P, mob/living/carbon/human/user, params)
	...
	if(istype(P, /obj/item/pen) || istype(P, /obj/item/toy/crayon))
		...
		to_chat(user, span_notice("I stamp the paper with my rubber stamp, however the ink ignites as you release the stamp."))
...
/obj/item/paper/contract/infernal/attack(mob/M, mob/living/user)
	...
	if(M == user && target == M.mind && M.mind.soulOwner != owner && attempt_signature(user, 1))
		user.visible_message(span_danger("[user] slices [user.p_their()] wrist with [src], and scrawls [user.p_their()] name in blood."), span_danger("I slice my wrist open and scrawl my name in blood."))
...
/obj/item/paper/contract/infernal/proc/attempt_signature(mob/living/carbon/human/user, blood = 0)
	...
	if(user.mind.soulOwner == owner)
		to_chat(user, span_warning("This devil already owns my soul, you may not sell it to [owner.p_them()] again!"))
		return 0
	if(signed)
	...
	if (contractType == CONTRACT_REVIVE)
		to_chat(user, span_warning("I are already alive, this contract would do nothing."))
		return 0
	else
		to_chat(user, span_notice("I quickly scrawl my name on the contract."))
...
/obj/item/paper/contract/infernal/proc/fulfillContract(mob/living/carbon/human/user = target.current, blood = FALSE)
	...
	to_chat(user, span_notice("A profound emptiness washes over you as you lose ownership of my soul."))
