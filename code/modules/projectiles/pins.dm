...
/obj/item/firing_pin/paywall/attack_self(mob/user)
	...
	to_chat(user, span_notice("I set the pin to [( multi_payment ) ? "process payment for every shot" : "one-time license payment"]."))
...
/obj/item/firing_pin/paywall/attackby(obj/item/M, mob/user, params)
	if(istype(M, /obj/item/card/id))
		...
		if(id != pin_owner && owned)
			...
			return
		if(id == pin_owner)
			to_chat(user, span_notice("I unlink the card from the firing pin."))
			gun_owners -= user
			pin_owner = null
			owned = FALSE
			return
		...
		gun_owners += user
		to_chat(user, span_notice("I link the card to the firing pin."))
