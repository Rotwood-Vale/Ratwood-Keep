...
/datum/status_effect/slimeskin/on_apply()
	...
	owner.visible_message(span_warning("[owner] is suddenly covered in a strange, blue-ish gel!"),
		span_notice("I are covered in a thick, rubbery gel."))
	return ..()
...
/datum/status_effect/stabilized/darkblue/tick()
	...
	if(istype(O, /obj/item/reagent_containers/food/snacks/monkeycube))
		to_chat(owner, span_warning("[linked_extract] kept my hands wet! It makes [O] expand!"))
	...
	// Dehydrated carp
		...
		to_chat(owner, span_warning("[linked_extract] kept my hands wet! It wets [O]!"))
	...
	if(owner.stat == DEAD)
		if(clone && clone.stat != DEAD)
			owner.visible_message(span_warning("[owner] blazes with brilliant light, [linked_extract] whisking [owner.p_their()] soul away."),
				span_notice("I feel a warm glow from [linked_extract], and you open my eyes... elsewhere."))
			if(owner.mind)
				owner.mind.transfer_to(clone)
			clone = null
			qdel(linked_extract)
		if(!clone || clone.stat == DEAD)
			to_chat(owner, span_notice("[linked_extract] desperately tries to move my soul to a living body, but can't find one!"))
			qdel(linked_extract)
	..()
...
/datum/status_effect/stabilized/green/on_remove()
	to_chat(owner, span_notice("I feel more like myself."))
	if(ishuman(owner))
		var/mob/living/carbon/human/H = owner
...
/datum/status_effect/stabilized/pink/tick()
	...
	if(HasFaction && owner.has_status_effect(/datum/status_effect/brokenpeace))
		...
		to_chat(owner, span_danger("The peace has been broken! Hostile creatures will now react to you!"))
	if(!HasFaction && !owner.has_status_effect(/datum/status_effect/brokenpeace))
...
/datum/status_effect/stabilized/black/tick()
	if(owner.pulling && isliving(owner.pulling) && owner.grab_state == GRAB_KILL)
		...
		if(M.stat == DEAD)
			return
		if(!messagedelivered)
			to_chat(owner,span_notice("I feel my hands melt around [M]'s neck and start to drain [M.p_them()] of life."))
			to_chat(owner.pulling, span_danger("[owner]'s hands melt around my neck, and you can feel my life starting to drain away!"))
