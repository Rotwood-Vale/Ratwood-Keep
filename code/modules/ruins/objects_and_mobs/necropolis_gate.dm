...
/obj/structure/necropolis_gate/legion_gate/attack_hand(mob/user)
	if(!open && !changing_openness)
		...
		if(safety == "Abort" || !in_range(src, user) || !src || open || changing_openness || user.incapacitated())
			return
		user.visible_message(span_warning("[user] knocks on [src]..."), span_boldannounce("I tentatively knock on [src]..."))
...
/obj/structure/necropolis_gate/legion_gate/toggle_the_gate(mob/user, legion_damaged)
	...
	if(.)
		locked = TRUE
		var/turf/T = get_turf(src)
		visible_message(span_danger("Something horrible emerges from the Necropolis!"))
		if(legion_damaged)
		...
		for(var/mob/M in GLOB.player_list)
			if(M.z == z)
				to_chat(M, span_danger("Discordant whispers flood your mind in a thousand voices. Each one speaks your name, over and over. Something horrible has been released."))
