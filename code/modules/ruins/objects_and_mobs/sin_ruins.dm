...
/obj/structure/cursed_slot_machine/interact(mob/living/carbon/human/user)
	...
	if(user.stat)
		to_chat(user, span_danger("No... just one more try..."))
		user.gib()
	else
		user.visible_message(span_warning("[user] pulls [src]'s lever with a glint in [user.p_their()] eyes!"), "<span class='warning'>I feel a draining as you pull the lever, but you \
