...
/obj/effect/proc_holder/spell/invoked/heatmetal/cast(list/targets, mob/user = usr)
	...
	if (!target || is_type_in_list(target, nosmeltore))
		return
	if (istype(target, /obj/item))
		handle_item_smelting(target, user, nosmeltore)
...
/proc/handle_item_smelting(obj/item/target, mob/user, list/nosmeltore)
	if (!target.smeltresult) return
	var/obj/item/itemtospawn = target.smeltresult
	user.visible_message("<font color='yellow'>After [user]'s incantation, [target] glows brightly and melts into an ingot.</font>",\
		"<font color='yellow'>After your incantation, [target] glows brightly and melts into an ingot.</font>")
	new itemtospawn(target.loc)
	do_sparks(1, 1, target.loc)
	qdel(target)
...
/proc/handle_tongs(obj/item/rogueweapon/tongs/T, mob/user) //Stole the code from smithing.
	...
	var/tyme = world.time
	T.hott = tyme
	addtimer(CALLBACK(T, TYPE_PROC_REF(/obj/item/rogueweapon/tongs, make_unhot), tyme), 10 SECONDS)
	T.update_icon()
...
/obj/effect/proc_holder/spell/invoked/vigorousexchange/cast(list/targets, mob/living/carbon/user = usr)
	...
	if (target == user)
		...
		show_visible_message(user, "As [user] intones the incantation, vibrant flames swirl around them.", "As you intones the incantation, vibrant flames swirl around you. You feel refreshed.")
...
/obj/effect/proc_holder/spell/invoked/craftercovenant/cast(list/targets, mob/user = usr)
	...
	var/static/list/sellable_typecache
	if(!sellable_typecache)
		sellable_typecache = typecacheof(list(/obj/item/roguecoin, /obj/item/roguestatue, /obj/item/clothing/ring, /obj/item/roguegem))
	for (var/obj/item/sacrifice in altar)
		if(is_type_in_typecache(sacrifice, sellable_typecache))
			tithe += sacrifice.get_real_price()
		qdel(sacrifice)
	...
	if (!doable.len)
		show_visible_message(user, "A wave of heat washes over the pile as [user] speaks Malum's name. The pile of valuables crumble into dust.", "A wave of heat washes over the pile as you speak Malum's name. The pile of valuables crumble into dust. Malum accepted your sacrifice. Yet it seems it wasn't enough.")
		return
...
// This is godawful. Please someone kill this.
/proc/initialize_anvil_recipe_prices()
	for (var/datum/anvil_recipe/recipe in GLOB.anvil_recipes)
		add_recipe_to_global(recipe)
	GLOB.anvil_recipe_prices += list(list(/obj/item/rogue/instrument/flute, 10))
	GLOB.anvil_recipe_prices += list(list(/obj/item/rogue/instrument/drum, 10))
	GLOB.anvil_recipe_prices += list(list(/obj/item/rogue/instrument/harp, 20))
	GLOB.anvil_recipe_prices += list(list(/obj/item/rogue/instrument/lute, 20))
	GLOB.anvil_recipe_prices += list(list(/obj/item/rogue/instrument/guitar, 30))
	GLOB.anvil_recipe_prices += list(list(/obj/item/rogue/instrument/accord, 30))
	GLOB.anvil_recipe_prices += list(list(/obj/item/riddleofsteel, 400))
	GLOB.anvil_recipe_prices += list(list(/obj/item/dmusicbox, 500))
...
/obj/effect/proc_holder/spell/invoked/hammerfall/cast(list/targets, mob/user = usr)
	...
	for (var/mob/living/carbon/shaken in view(radius, fallzone))
		...
		if (diceroll > dc)
			...
			to_chat(shaken, "<font color='yellow'>The ground quakes but I manage to keep my footing.</font>")
		else
			shaken.apply_effect(1 SECONDS, EFFECT_KNOCKDOWN, 0)
			to_chat(shaken, "<font color='yellow'>The ground quakes, making me fall over.</font>")
