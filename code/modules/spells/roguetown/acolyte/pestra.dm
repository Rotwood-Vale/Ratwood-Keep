...
/obj/effect/proc_holder/spell/invoked/diagnose/cast(list/targets, mob/living/user)
	if(ishuman(targets[1]))
		...
		user.log_message("checked [key_name(human_target)] for injuries with [miracle ? "" : "Secular"] Diagnose.", LOG_ATTACK)
		human_target.log_message("was checked for injuries using [miracle ? "" : "Secular"] Diagnose by [key_name(user)].", LOG_ATTACK)
		return TRUE
	return FALSE
...
/obj/effect/proc_holder/spell/invoked/attach_bodypart/proc/get_organs(mob/living/target, mob/living/user)
	...
	for(var/obj/item/organ/potential_organ in user?.held_items)
		...
		user.log_message("reattached [key_name(target)]'s [potential_organ.slot] using Bodypart Miracle.", LOG_ATTACK)
		target.log_message("had their [potential_organ.slot] reattached using Bodypart Miracle by [key_name(user)].", LOG_ATTACK)
	//then target's hands
	for(var/obj/item/organ/dismembered in target.held_items)
		if(dismembered.owner || !(dismembered.slot in missing_organs))
			continue
		organs += dismembered
		user.log_message("reattached [key_name(target)]'s [dismembered.slot] using Bodypart Miracle.", LOG_ATTACK)
		target.log_message("had their [dismembered.slot] reattached using Bodypart Miracle by [key_name(user)].", LOG_ATTACK)
...
/obj/effect/proc_holder/spell/invoked/attach_bodypart/cast(list/targets, mob/living/user)
	if(ishuman(targets[1]))
		...
		for(var/obj/item/bodypart/limb as anything in get_limbs(human_target, user))
			...
			user.log_message("reattached [key_name(human_target)]'s [limb] using Bodypart Miracle.", LOG_ATTACK)
			human_target.log_message("had their [limb] reattached using Bodypart Miracle by [key_name(user)].", LOG_ATTACK)
		for(var/obj/item/organ/organ as anything in get_organs(human_target, user))
			...
			user.log_message("reattached [key_name(human_target)]'s [organ] using Bodypart Miracle.", LOG_ATTACK)
			human_target.log_message("had their [organ] reattached using Bodypart Miracle by [key_name(user)].", LOG_ATTACK)
...
/obj/effect/proc_holder/spell/invoked/cure_rot/cast(list/targets, mob/living/user)
	if(isliving(targets[1]))
		...
		if(!HAS_TRAIT(target, TRAIT_ROTMAN))
			...
			user.log_message("cured [key_name(target)]'s rot using Cure Rot.", LOG_ATTACK)
			target.log_message("had their rot cured using Cure Rot by [key_name(user)].", LOG_ATTACK)
		else
			target.visible_message(span_warning("The rot fails to leave [target]'s body!"), span_warning("I feel no different..."))
			user.log_message("cast Cure Rot on [key_name(target)] but remained a rotman.", LOG_ATTACK)
			target.log_message("had Cure Rot cast on them by [key_name(user)] but remained a rotman.", LOG_ATTACK)
		return TRUE
	return FALSE
