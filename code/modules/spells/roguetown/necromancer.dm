...
/obj/effect/proc_holder/spell/invoked/raise_lesser_undead/cast(list/targets, mob/living/user)
	...
	if(isopenturf(T))
		switch(skeleton_roll)
			if(1 to 20)
				new /mob/living/simple_animal/hostile/retaliate/skeleton/axe(T)
			if(21 to 40)
				new /mob/living/simple_animal/hostile/retaliate/skeleton/spear(T)
			if(41 to 60)
				new /mob/living/simple_animal/hostile/retaliate/skeleton/guard(T)
			if(61 to 80)
				new /mob/living/simple_animal/hostile/retaliate/skeleton/bow(T)
			if(81 to 100)
				new /mob/living/simple_animal/hostile/retaliate/skeleton(T)
		return TRUE
	else
		to_chat(user, span_warning("The targeted location is blocked. My summon fails to come forth."))
		return FALSE
