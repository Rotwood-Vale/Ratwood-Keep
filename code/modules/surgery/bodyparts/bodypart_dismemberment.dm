...
/obj/item/bodypart/proc/dismember(dam_type = BRUTE, bclass = BCLASS_CUT, mob/living/user, zone_precise = src.body_zone)
	...
	if(body_zone == BODY_ZONE_HEAD)
		...
		C.visible_message(span_warning("<B>The [src.name] is [pick("torn off", "sundered", "severed", "seperated", "unsewn")]!</B>"))
		C.emote("painscream")
	src.add_mob_blood(C)
...
/obj/item/bodypart/chest/dismember(dam_type = BRUTE, bclass = BCLASS_CUT, mob/living/user, zone_precise = src.body_zone)
	...
	if(organ_spilled)
		C.visible_message(span_danger("<B>[C] spills [C.p_their()] guts!</B>"))
	return TRUE
