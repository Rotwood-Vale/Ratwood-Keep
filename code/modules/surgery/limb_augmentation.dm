...
/datum/surgery_step/replace/preop(mob/user, mob/living/carbon/target, target_zone, obj/item/tool, datum/surgery/surgery)
	display_results(user, target, span_notice("I begin to sever the muscles on [target]'s [parse_zone(user.zone_selected)]..."),
		span_notice("[user] begins to sever the muscles on [target]'s [parse_zone(user.zone_selected)]."),
		span_notice("[user] begins an incision on [target]'s [parse_zone(user.zone_selected)]."))
