...
/obj/vehicle/sealed/car/clowncar/Bump(atom/movable/M)
	...
	if(isliving(M))
		...
		L.visible_message(span_warning("[src] rams into [L] and sucks [L.p_them()] up!")) //fuck off shezza this isn't ERP.
		mob_forced_enter(L)
		playsound(src, pick('sound/blank.ogg'), 75)
...
/obj/vehicle/sealed/car/clowncar/emag_act(mob/user)
	if(obj_flags & EMAGGED)
		return
	obj_flags |= EMAGGED
	to_chat(user, span_danger("I scramble \the [src]'s child safety lock, and a panel with six colorful buttons appears!"))
