...
/obj/item/clothing/mask/cigarette/rollie/snek
	list_reagents = list(/datum/reagent/drug/nicotine = 30, /datum/reagent/consumable/honey = 30, /datum/reagent/toxin/venom = 2) //honey will give heal. it is NOT great 55% chance per unit tick to heal 1brute,burn,tox. high end perfect rolls 30hp. unlikely. degrades into sugar (chance) which has a chance to degrade into nutrients. simulates appetite surpressant effect.
...
/obj/item/reagent_containers/hypospray/medipen/snekbt
	...
	list_reagents = list(/datum/reagent/drug/snekbt = 5, /datum/reagent/toxin/venom = 2)
	to_chat(user, SPAN_NOTICE(" You inject [M] with [src]."))
	to_chat(M, SPAN_WARNING("You are stabbed thru the heart with a needle!"))
	playsound(src, 'inject.ogg', 25, TRUE)
...
/datum/reagent/drug/snekbt/overdose_start(mob/living/M, can_overdose = TRUE)
	if(HAS_TRAIT(M, TRAIT_CRACKHEAD))// boathian bullshit isnt going to help you here. its a toxin not a drug
		can_overdose = TRUE
	to_chat(M, SPAN_WARNING("you really..... really shouldnt of done that..."))
	M.ForceContractDisease(new /datum/disease/heart_failure(), FALSE, TRUE)
	ADD_TRAIT(M, TRAIT_PARALYSIS_L_ARM, TRAIT_PARALYSIS_L_LEG)
...
/datum/status_effect/buff/snekbt/on_remove()
	...
	REMOVE_TRAIT(owner, TRAIT_SCREENSHAKE, TRAIT_GENERIC)
	. = ..()
