...
obj/item/mundane/puzzlebox/easy
	...
	icon_state = "wood_box"
	var/fluff_desc = pick("It, frankly, looks rather depressing.","I can see an engraving of Psydon sending the Comet Syon on the side.","It doesn't look so difficult.","It's dusty and boring.","Why do I want to play with this for hours?","I could probably get a vagrant to solve this.","It looks like it was made for fools.")
...
obj/item/mundane/puzzlebox/easy/attack_self(mob/user)
	var/ckey = user.ckey
	...
	var/alert = alert(user, "My fingers trace the outside of this box. It looks easy. Do I try to solve it? \n[usage_prompt]", "wooden puzzle-box", "Yes", "No")
		...
		if(do_after(user,50, target = src))
			if((dice_roll) - 5 <= user.STAINT)
				to_chat(user, span_notice("I solve \the [src] fairly easily."))
...
obj/item/mundane/puzzlebox/medium
	...
	icon = 'modular_azurepeak/icons/obj/items/mundanities.dmi'
	icon_state = "ebon_box"
	var/fluff_desc = pick("Its surface shines with polished ebony.","I can see an engraving of a Snow-Elf on the side.","It looks like it could challenge an average man.","I wish my personality was like this box's.","Why do I want to play with this for hours?","I could probably sell this to a wizard's apprentice.","It looks...sufficient.")
	var/list/finished_ckeys = list()
	var/dice_roll = rand(10,20)

obj/item/mundane/puzzlebox/medium/Initialize()
	. = ..()
	desc += " [fluff_desc]"

obj/item/mundane/puzzlebox/medium/attack_self(mob/user)
	var/ckey = user.ckey
	if(ckey in finished_ckeys)
		to_chat(user, span_warning("I've already tried my hand at \the [src].")
	var/alert = alert(user, "My fingers trace the outside of this box. It looks of average difficulty. Do I try to solve it? \n[usage_prompt]", "wooden puzzle-box", "Yes", "No")
		if(alert != "Yes")
			return
...
obj/item/mundane/puzzlebox/impossible //literally nearly impossible to solve - if you do, you get a fairly lengthy buff or a stat boost.
	...
	var/fluff_desc = pick("It, frankly, looks nearly impossible.","Its centerpiece is that of Astrata banishing a heretic from this world.","Without doubt, this is rather befuddling.","It looks arcane and nearly-impossible.","Why do I feel like I could try for hours and not succeed at this?","I could probably sell this to a bored wizard.","It looks nearly impossible.")
	var/list/finished_ckeys = list()
	var/dice_roll = rand(10,20)
...
obj/item/mundane/puzzlebox/impossible/attack_self(mob/user)
	var/ckey = user.ckey
	if(ckey in finished_ckeys)
		to_chat(user, span_warning("I've already tried my hand at \the [src].")
